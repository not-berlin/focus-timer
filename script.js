const TRANS = {
  vi: {
    ...
    dropZoneImg:'Kéo thả ảnh, video hoặc thư mục vào đây', dropZoneImgSub:'hỗ trợ ảnh nền nhiều tấm hoặc 1 video nền',
  ...
  en: {
    ...
    dropZoneImg:'Drag & drop images, videos, or folders here', dropZoneImgSub:'supports image slideshows or one background video',
...
/* ── MUSIC TABS ── */
function switchMusicTab(tab, btn) { document.querySelectorAll('.music-tab').forEach(b => b.classList.remove('active')); btn.classList.add('active'); document.getElementById('music-tab-yt').style.display = tab === 'yt' ? 'block' : 'none'; document.getElementById('music-tab-sc').style.display = tab === 'sc' ? 'block' : 'none'; document.getElementById('music-tab-local').style.display = tab === 'local' ? 'block' : 'none'; document.getElementById('yt-frame-wrap').style.display = tab === 'local' ? 'none' : ''; }
